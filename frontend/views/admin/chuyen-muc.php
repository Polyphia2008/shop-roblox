...
$sourceCodes = $VCD->get_list("SELECT * FROM `chuyenmuc` ORDER BY id DESC");
...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0"><i class="fa-solid fa-tags"></i> Chuyên mục acc</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item active" aria-current="page"> Chuyên mục acc</li>
                    </ol>
                </nav>
            </div>
        </div>
                    ...
                    <div class="card-header justify-content-between">
                        ...
                        <button type="button" data-bs-toggle="modal" data-bs-target="#exampleModalScrollable2"
                            class="btn btn-sm btn-primary shadow-primary"><i
                                class="ri-add-line fw-semibold align-middle"></i> Tạo chuyên mục</button>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive mb-3">
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                     <?php $i=0; foreach ($VCD->get_list("SELECT * FROM `chuyenmuc` WHERE $where $order_by LIMIT $from,$limit") as $row) {?>
                                                                        <tr>
                                        <td><b>[<?=$row['id'];?>]</b>
                                        </td>
                                        <td class="text-center"><img src="<?=$row['logo'];?>" style="border-radius: 10px;" alt="pic" height="80px"></td>
                                        ...
                                        <td class="text-center"><span style="font-size: 15px;"
                                             ...
                                             <td class="text-center"><?=$row['title'];?></td>
                                              <td class="text-center"><?=format_cash($row['price']);?> đ</td>
                                                  <td class="text-center"><?=format_cash($row['buy']);?></td>
        ...
        document.getElementById('status<?= $row['id']; ?>').addEventListener('change', function() {
            ...
            <?php } else if ($i <= 4 || $i > $totalPages - 4 || abs($i - $page) <= 1) { ?>
                <li class="page-item <?php if($i == $page) { echo'active';}?>">
                    <a class="page-link link" href="/admin/chuyen-muc?id=<?=$id;?>&code=<?=$code;?>&limit=<?=$limit;?>&shortByDate=&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                </li>
            ...
            <?php } ?>
        <?php } ?>
        <li class="paginate_button page-item previous "><a class="page-link" href="/admin/chuyen-muc?id=<?=$id;?>&code=<?=$code;?>&limit=<?=$limit;?>&shortByDate=&page=<?=$page + 1;?>">Next</a></li>
                                                </ul></div></center>                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title" id="staticBackdropLabel2"><i class="fa-solid fa-plus"></i> Tạo chuyên mục
                </h6>
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    <div class="row mb-4">
                        <label class="col-sm-4 col-form-label" for="example-hf-email">Tên chuyên mục (<span
                      ...
                      <div class="row mb-4">
                        <label class="col-sm-4 col-form-label" for="example-hf-email">Logo image (<span
                                class="text-danger">*</span>)</label>
                        <div class="col-sm-8">
                            <div class="input-group">
                                <input type="url" class="form-control" name="logo" id="logo" required>
                                <span class="input-group-text">
                                    <i class="fa-solid fa-code"></i>
                                </span>
                            </div>
...
<script>
  $("#btn-chuyemuc").click(function() {
        $('#btn-chuyemuc').html('Loading...').prop('disabled',
            true);
        $.ajax({
            url: '<?=BASE_URL('ajaxs/admin/chuyen-muc.php')?>',
            type: 'POST',
            dataType: "json",
            data: {
                    action: "add",
