...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id = xss($_POST['id']);
    $row = $VCD->get_row(" SELECT * FROM `accountorder` WHERE `id` = '$id' AND `status`='1' ");
    if (!$row) {
        die(json_encode([
            'status'    => 'error',
  ...
  if (empty($row['username'] === NULL || $row['username'] === "" || $row['magd'] === NULL || $row['magd'] === "")) {
    ...
    'giaohang'     => gettime(),
      ...
      notiTele("🔔 Báo Cáo #{$magd} 🔔\nThanh toán đơn order robux với giá " . format_cash($total) . 'đ thành công! Lúc: ' . date('d/m/Y H:i:s'), $getUser['telegram']);
