...
if (isset($_POST['update_status']) && isset($_POST['id'])) {
    ...
    if ($ticket) {
            $ticket_id = $VCD->get_row("SELECT * FROM ticket WHERE id = '".$id."' AND  `dichvu`='order'");
        $accountrb = $VCD->get_row("SELECT * FROM accountrb WHERE id = '".$ticket_id['nickrb']."'");
        if ($accountrb) {
            $userGuest = $VCD->get_row("SELECT * FROM users WHERE email = '$accountrb[username]'");
            if ($userGuest) {
           notiTele("🔔 Báo Cáo Order #{$accountrb['magd']} 🔔\nTrạng thái khiếu nại của quý khách đã được duyệt vui lòng kiểm tra lại! Lúc: " . date('d/m/Y H:i:s'), $userGuest['telegram']);
            }
        }
        echo "<script>
            Swal.fire({
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive mb-3">
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                     <?php $i=0; foreach ($VCD->get_list("SELECT * FROM `ticket` WHERE  `dichvu`='order'  AND $where $order_by LIMIT $from,$limit") as $row) {?>
                                                                        <tr>
                                        <td><b>[<?=$row['id'];?>]</b>
                                        </td>
        ...
        document.getElementById('status<?= $row['id']; ?>').addEventListener('change', function() {
            ...
            <?php } else if ($i <= 4 || $i > $totalPages - 4 || abs($i - $page) <= 1) { ?>
                <li class="page-item <?php if($i == $page) { echo'active';}?>">
                    <a class="page-link link" href="/admin/ticket-order?id=<?=$id;?>&code=<?=$code;?>&limit=<?=$limit;?>&shortByDate=&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                </li>
            ...
            <?php } ?>
        <?php } ?>
        <li class="paginate_button page-item previous "><a class="page-link" href="/admin/ticket-order?id=<?=$id;?>&code=<?=$code;?>&limit=<?=$limit;?>&shortByDate=&page=<?=$page + 1;?>">Next</a></li>
                                                </ul></div></center>                                    </div>
                                </div>
                            </div>
                        </div>
