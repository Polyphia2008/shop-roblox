...
?>
    ...
    <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
        <div class="grow">
            <h5 class="text-16">Lịch sử mua</h5>
        </div>
    </div>
     <!--mã nguồn được phát triển bởi Văn Công Đức ✓-->
        ...
        <div class="col-span-12 card 2xl:col-span-12">
            <div class="card-body">
                ...
                <div class="overflow-x-auto">
                    <div class="grid items-center grid-cols-1 gap-3 mb-5 2xl:grid-cols-12">
                        <div class="2xl:col-span-5 flex flex-wrap">
                            <div class="overflow-x-auto mb-2 mr-2">
    <button type="button" data-id="table-pre" x-bind="exportExcel" class="relative rounded mt-1 text-white btn py-1 px-0 bg-green-500 border-green-500 hover:text-white hover:bg-green-600 hover:border-green-600 focus:text-white focus:bg-green-600 focus:border-green-600 focus:ring focus:ring-green-100 active:text-white active:bg-green-600 active:border-green-600 active:ring active:ring-green-100 dark:ring-green-400/20">
        Excel
    </button>
</div>
                            <div class="overflow-x-auto mb-2">
                                                                    <button type="button" data-modal-target="modalTips" class="relative rounded mt-1 text-white btn py-1 px-0 bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                        Hướng dẫn login acc có 2FA Auth
                                    </button>
                                                                <button type="button" data-modal-target="useBoot" class="relative rounded mt-1 text-white btn py-1 px-0 bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                    Cách sử dụng bot
                                </button>
                                <button type="button" data-modal-target="howToReport" class="relative rounded mt-1 text-white btn py-1 px-0 bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                    Hướng dẫn sử dụng report
                                </button>
                            </div>
                        </div>
                                                    <div class="2xl:col-span-7">
                                <marquee class="tw-w-full " scrollamount="2">
                                    <p class="font-bold" style="color: red">Mua acc vui lòng kiểm tra tài khoản mật khẩu trong 5 phút, nếu có lỗi vui lòng sử dụng nút khiếu nại, không sử dụng này shop không hỗ trợ bảo hành.</p>
                                </marquee>
                            </div>
                                            </div>
                        ...
                        <tbody>
                               <?php $i = 0;
$sqlOrder = "SELECT * FROM `accountorder` WHERE `username` = '".$getUser['email']."' AND `status` != '1'";
$listOrder = $VCD->get_list($sqlOrder);
$sqlAcc = "SELECT * FROM `accountrb` WHERE `username` = '".$getUser['email']."' AND `status` = '2'";
$listAcc = $VCD->get_list($sqlAcc);
$list_history = array_merge($listOrder, $listAcc);

foreach ($list_history as $rowacc) {
                               ?>
                               <?php
                              $accountrb = json_decode($rowacc['information'], true);
                                 ?>
    ...
    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                           ...
                           <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                        <?=$accountrb['tknick'];?>
                           <button onclick="copyText('tknick')" class="text-blue-500 ml-2">
                             <i class="far fa-copy" style="   color: rgba(0, 0, 0, 0.6); "></i>
                        </button>
                         </td>
                             <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                           <?=$accountrb['pass'];?>
                            <button onclick="copyText('pass')" class="text-blue-500 ml-2">
                         <i class="far fa-copy" style="   color: rgba(0, 0, 0, 0.6); "></i>
                           </button>
                                 </td>
                                    ...
                                    <?=format_cash($rowacc['robux']);?> 
                                    </td>
                                       <td class="px-3.5 py-2.5 first:pl-5  last:pr-5 border-y border-slate-200 dark:border-zink-500">

                                    <?=format_cash($rowacc['price']);?> đ
                                    </td>
                                     <td class="px-3.5 py-2.5 first:pl-5  last:pr-5 border-y border-slate-200 dark:border-zink-500">

                                    <?=$rowacc['guarantee'];?> Phút
                                    </td>
                                      <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                                            <?=$accountrb['2fa'];?> 
                                   ...
                                   <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
    <button type="button" data-modal-target="detailOrder<?=$rowacc['id'];?>" class="relative rounded mt-1 text-white btn py-1 px-0 bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
        <i class="fa fa-eye"></i> Chi tiết
    </button>
</td>
   <div id="detailOrder<?=$rowacc['id'];?>" wire:ignore.self="" modal-center="" class="fixed flex flex-col transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show hidden">
        <div class="w-screen lg:w-[55rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
            <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
                ...
                <button data-modal-close="detailOrder<?=$rowacc['id'];?>" type="button" class="transition-all duration-200 ease-linear text-slate-500 hover:text-red-500 dark:text-zink-200 dark:hover:text-red-500">
                ...
                <label class="">Chi tiết</label>
                                   ...
                                   <h4 class="text-16">Báo lỗi</h4>
                                    ...
                                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                                     <?php $rowticket = $VCD->get_row("SELECT * FROM `ticket` WHERE `nickrb` = '" . $rowacc['id'] . "'");
                                            if ($rowticket)  {?>
                                    <strong >Báo lỗi</strong>: <label class=""><?=report($rowticket['type']);?></label> <br>
                                    <label class="">Lý do: <?=$rowticket['lydo'];?></label>
                                             <?php } ?> 
                                             </td>
                                                  <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                                                      <?php if($rowacc['status'] == '2'){ ?>
                                         <span class="relative rounded mt-1 text-white btn py-1 px-0 success-ducapi">
   Thành công
</span>

<?php } else if($rowacc['status'] == '3') { ?>
<span class="relative rounded mt-1 text-white btn py-1 px-0 err-ducapi">
   Đang xử lý
</span>
<?php }?>
                                                  ...
                                                  <?php  
$time_row = strtotime($rowacc['time']); 
$time_limit = $time_row + 600;


    $rowreport = $VCD->get_row("SELECT * FROM `ticket` WHERE `nickrb` = '" . $rowacc['id'] . "'");
 echo status_report($rowreport['status']);
 if (time() <= $time_limit) {
    if ($rowreport) {
        $ticket = $VCD->get_row("SELECT * FROM `ticket` WHERE `nickrb` = '" . $rowacc['id'] . "' AND `status` != '1'");

        if (empty($ticket)) { ?>
            <br>
            <button type="button" name="HuyReport" onclick="HuyReport(<?= $rowacc['id']; ?>)" 
                class="relative rounded mt-1 text-white btn py-1 px-0 bg-custom-500 border-custom-500 
        ...
        ?>
        <button type="button" data-modal-target="modalReport<?= $rowacc['id']; ?>" 
            class="relative rounded mt-1 text-white btn py-1 px-0 bg-red-500 border-red-500 
            hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white 
            focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 
            active:text-white active:bg-red-600 active:border-red-600 active:ring 
...
?>
                                      ...
                                      <div id="modalReport<?= $rowacc['id']; ?>" wire:ignore.self modal-center="" class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
    <form method="post" wire:submit="report">
        <div class="w-screen md:w-[40rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
            <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
                ...
                <button data-modal-close="modalReport<?= $rowacc['id']; ?>" type="button" class="transition-all duration-200 ease-linear text-slate-500 hover:text-red-500 dark:text-zink-200 dark:hover:text-red-500">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-lucide="x" class="lucide lucide-x size-5"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg>
                </button>
            </div>
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                 <input type="hidden" id="token" value="<?= empty($getUser['token']) ? '' : $getUser['token'] ?>">
                    <input type="hidden" id="dichvu" value=" <?=$accountrb['dichvu'];?>">
