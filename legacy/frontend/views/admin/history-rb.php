...
$sourceCodes = $VCD->get_list("SELECT * FROM `accountrb` ORDER BY id DESC");
...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0"><i class="fa-solid fa-tags"></i> Nick Có Robux</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item active" aria-current="page">Nick Có Robux</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="col-xl-12">
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        ...
                        <a  href="/admin/add-nick-robux"
                            class="btn btn-sm btn-primary shadow-primary"><i
                                class="ri-add-line fw-semibold align-middle"></i> Thêm nick có robux</a>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row row-cols-lg-auto g-3 mb-3">
                                ...
                                <div class="col-12">
                                    <button class="btn btn-hero btn-sm btn-primary"><i class="fa fa-search"></i>
                                        Search                                    </button>
                                    <a class="btn btn-hero btn-sm btn-danger" href="/admin/nick-robux"><i
                                            class="fa fa-trash"></i>
                                        Clear filter                                    </a>
                                </div>
                            </div>
                        ...
                        <div class="table-responsive mb-3">
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                 <?php 
$i = 0; 
foreach ($VCD->get_list("SELECT * FROM `accountrb` WHERE username IS NOT NULL AND username != '' AND $where $order_by LIMIT $from, $limit") as $rowacc) {
?>
                                       ...
                                       Premium: <?=premium($rowacc['premium']);?> <br>Ngày tham gia: <?=$rowacc['datejoin'];?> 
                                        </td>
                                        <td class="text-center">
                                              <a href="<?=BASE_URL('admin/edit-nick-robux/')?><?=$rowacc['id']?>" class="btn btn-sm btn-primary shadow-primary btn-wave waves-effect waves-light" data-bs-toggle="tooltip" data-bs-original-title="Edit">
            ...
            <?php } else if ($i <= 4 || $i > $totalPages - 4 || abs($i - $page) <= 1) { ?>
                <li class="page-item <?php if($i == $page) { echo'active';}?>">
                    <a class="page-link link" href="/admin/history-rb?id=<?=$id;?>&code=<?=$code;?>&limit=<?=$limit;?>&shortByDate=&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                </li>
            ...
            <?php } ?>
        <?php } ?>
        <li class="paginate_button page-item previous "><a class="page-link" href="/admin/history-rb?id=<?=$id;?>&code=<?=$code;?>&limit=<?=$limit;?>&shortByDate=&page=<?=$page + 1;?>">Next</a></li>
                                                </ul></div></center>                                    </div>
                                </div>
                            </div>
                        </div>
...
function RemoveRow(id) {
    cuteAlert({
        ...
        if (e) {
            $.ajax({
                url: "<?=BASE_URL('')?>ajaxs/admin/remove_robux.php",
                method: "POST",
                dataType: "JSON",
                data: {
                    id: id,
