...
$rowbank = $VCD->get_row("SELECT * FROM `bank` WHERE `token` = '" . check_string($_GET['token']) . "' AND `short_name` = 'MBBANK'");
if (!$rowbank) {
    echo 'Lỗi rồi';
}
$response = file_get_contents("https://api.sieuthicode.net/historyapimbbankv2/" . $rowbank['token']);
...
$bankid = explode("-", $bankid)[0];
        if ($bankid) {
            $getDon = $VCD->get_row("SELECT * FROM don_nap WHERE noidung = '$bankid' AND `status` = 'xuly'");
            if ($getDon) {
                ...
                if ($insertSv2) {
                    $received = $amount;
                    $isCong = PlusCredits($getDon['userid'], $received, "Nạp tiền tự động qua MBBANK (#$tid - $description - $amount)");
