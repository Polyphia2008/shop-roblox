...
$sourceCodes = $VCD->get_list("SELECT * FROM `rateorder` ORDER BY id DESC");
...
if (isset($_POST['update_status']) && isset($_POST['id'])) {
    ...
    echo "<script>
        Swal.fire({
            ...
            setTimeout(function() {
                window.location.href = '/admin/order-rate'; 
            }, 500); 
        });
    </script>";
}
...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0"><i class="fa-solid fa-tags"></i> Danh sách Rate Order</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item active" aria-current="page"> Rate Order</li>
                    </ol>
                </nav>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive mb-3">
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                     <?php $i=0; foreach ($VCD->get_list("SELECT * FROM `rateorder` WHERE $where $order_by LIMIT $from,$limit") as $row) {?>
                                                                        <tr>
                                        <td><b>[<?=$row['id'];?>]</b>
                                        </td>
        ...
        document.getElementById('status<?= $row['id']; ?>').addEventListener('change', function() {
            ...
            <?php } else if ($i <= 4 || $i > $totalPages - 4 || abs($i - $page) <= 1) { ?>
                <li class="page-item <?php if($i == $page) { echo'active';}?>">
                    <a class="page-link link" href="/admin/order-rate?id=<?=$id;?>&code=<?=$code;?>&limit=<?=$limit;?>&shortByDate=&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                </li>
            ...
            <?php } ?>
        <?php } ?>
        <li class="paginate_button page-item previous "><a class="page-link" href="/admin/order-rate?id=<?=$id;?>&code=<?=$code;?>&limit=<?=$limit;?>&shortByDate=&page=<?=$page + 1;?>">Next</a></li>
                                                </ul></div></center>                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
  $("#AddMucRate").click(function() {
        $('#AddMucRate').html('Loading...').prop('disabled',
            true);
        $.ajax({
            url: '<?=BASE_URL('ajaxs/admin/rateorder.php')?>',
            type: 'POST',
            dataType: "json",
            data: {
                action: "add",
